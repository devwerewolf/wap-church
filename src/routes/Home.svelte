<script lang="ts">
  import { ACCESS_KEY } from "../env/unsplash";
  
  async function getRandomAnimals(count = 3) {
    const unsplashEndpoint = `https://api.unsplash.com/photos/random?client_id=${ACCESS_KEY}&count=${count}&auto=format&fit=crop&w=800&h=600&q=60&orientation=landscape&query=animal`;
    
    const animals = await fetch(unsplashEndpoint)
    return await animals.json() as Array<any>;
  }
</script>
